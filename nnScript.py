...
def nnObjFunction(params, *args):
    ...
    for i in range(0, lim):
        ...
        for l in range(0, n_class):
            JW1W2i -= (training_label[i] * np.log(oi[l]) + (1 - training_label[i]) * np.log(1 - oi[l]))
   
        JW1W2 += np.sum((1/len(training_data[:,0])) * JW1W2i)
        Si = np.subtract(np.max(oi), training_label[i]);
        ...
        dJiW1p2 = np.sum(np.subtract((np.max(oi)* w2), (training_label[i]* w2)))
